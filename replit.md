...
- **일정 유형 선택 방식** (성과제/연가/포상휴가/위로휴가/기타휴가/외출) 추가/수정/삭제 (본인 일정만)
  - 기타휴가 선택 시 직접 입력 가능 (병가, 청원휴가 등)
  - 달력 타일에 유형별 색상 + 앞글자 표시
- 친구 캘린더 보기 (친구 탭 → 📅 일정 버튼)
- 군 복무 진행률 바
...
- `users` — 회원 정보
- `schedules` — 일정 (`schedule_type`, `custom_type` 컬럼 포함)
